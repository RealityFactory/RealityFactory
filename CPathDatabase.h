...
public:
	...
	int Locate(char *szPointName, int *nType, geVec3d *Position, geFloat *fRange);	// Find a point in the database
	int OpenPath(char *szPointName, geVec3d *Position);								// Find first point in the database,
	...
	int GetPointRange(int nPathHandle, geFloat *fRange);							// If current point ranged, get range
	int ClosePath(int nPathHandle);													// Release path handle

private:
	PathDatabaseRecord *CreateNewPath(char *szPointName, const geVec3d &Position,
									int nType, geFloat Range);
	int AddToPath(PathDatabaseRecord *pHead, char *szPointName, const geVec3d &Position,
