...
//	Constructor
//
//	Set all morphing fields to default values, load sounds, set up fog
//	..zones.  If there is audio, start it looping at zero volume.
...
CMorphingFields::CMorphingFields()
{
	...
	pEntity= geEntity_EntitySetGetNextEntity(pSet, pEntity)) 
	{
		...
		pField->bForward = true;							// Animation direction
...
void CMorphingFields::Tick(geFloat dwTicks)
{
	...
	pEntity= geEntity_EntitySetGetNextEntity(pSet, pEntity)) 
	{
		...
		if(pField->active==GE_TRUE)
		{
			...
			{
				CCD->PathFollower()->GetNextPosition(pField->szEntityName, &pField->origin,
					false);				// Seek to next position, if in motion
			}
			else
			{
			...
			if(pField->theFog != NULL)
			{
				...
				case true:
					...
					if(pField->clrCurrent.r < 0.0) pField->clrCurrent.r = 0.0;
					if(pField->clrCurrent.g > 255.0) pField->clrCurrent.g = 255.0;
					if(pField->clrCurrent.g < 0.0) pField->clrCurrent.g = 0.0;
					if(pField->clrCurrent.b > 255.0) pField->clrCurrent.b = 255.0;
					if(pField->clrCurrent.b < 0.0) pField->clrCurrent.b = 0.0;
					...
					{
						pField->bForward = false;								// Switch direction
						pField->tElapsed = (geFloat)pField->fogSpeed;
				...
				case false:
					...
					if(pField->tElapsed <= 0)
					{
						pField->bForward = true;								// Switch direction
						pField->tElapsed = (geFloat)pField->fogSpeed;
