...
//	Constructor
//
//	Initialize the list of active particle systems to empty, then go
//	..through the loaded world and set up all the static particle
//	..systems defined therein.
...
// end change RF064
		...
		switch(pProxy->nStyle)
		{
			...
			case kPSystem_ImplodeSpiralArm:
				if(Create(&psHandle, pProxy->nStyle, pmHandle, pProxy->origin, pProxy->Scale,
										pProxy->szEntityName) != RGF_SUCCESS)
				  {
					CCD->ReportError("Can't create particle system from proxy", false);
			  ...
			  // Check for system default over-rides in the proxy
				...
				break;
			default:
				CCD->ReportError("Illegal particle system style", false);
				return;											// Another serious error.
		}
		pProxy->active=false;
		pProxy->effect = (int *)malloc(sizeof(int) * 1);
		pProxy->effect[0] = -1;
...
int CParticleSystem::CreateSound(geVec3d Origin, char *SoundFile, float radius)
{
	int effect = -1;
	Snd Sound;
...
int CParticleSystem::LoadParticleMap(int *pmHandle, char *szPrimary,
			char *szAlpha)
{
  int nHandle = (-1), nTemp;
...
//	We have a handle, let's load the files.
	...
	if(bmpList[nHandle] == NULL)
	  {
		char szBug[200];
		sprintf(szBug,"Can't open particlemap file %s", szPrimary);
		CCD->ReportError(szBug, false);
		return RGF_FAILURE;
		}
...
//	Create
//
//	Create a brand-new particle system.  This particle system becomes
//	..active immediately on return from this routine and will be eligible
//	..for display in the next Render() call.  A handle to the particle
//	..system is returned if it created successfully.  Note that all
//	..systems are created NOT RUNNING and HIDDEN.

int CParticleSystem::Create(int *psHandle, int nSystemType, int nTexHandle,
			geVec3d emitterPos, float Scale, char *szName)
{
	int nHandle = -1;
  ...
  switch(nSystemType)
	  ...
	  case kPSystem_Shockwave:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, SHOCKWAVE", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 5000;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 5000;	// Whole system life
	  ...
	  case kPSystem_Fountain:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, FOUNTAIN", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 5000;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 22000;	// Whole system life
	  ...
	  case kPSystem_Rain:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, RAIN", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 5000;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 30000;	// Whole system life
	  ...
	  case kPSystem_Sphere:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, SPHERE", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 2000;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 20000;	// Whole system life
			theList[nHandle]->Scale = 0.03f*Scale;			// Small particles
			theList[nHandle]->bRunning = false;		// Create paused
			theList[nHandle]->bVisible = false;		// Create invisible
			theList[nHandle]->Texture = bmpList[nTexHandle];
	  ...
	  case kPSystem_Column:
		  ...
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, COLUMN", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 3500;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 16000;	// Whole system life
	  ...
	  case kPSystem_ExplosiveArray:
		  ...
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, EXPLOSIVE ARRAY", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 3000;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 8000;	// Whole system life
			theList[nHandle]->Scale = 0.05f*Scale;			// Small particles
			theList[nHandle]->bRunning = false;		// Create paused
			theList[nHandle]->bVisible = false;		// Create invisible
			theList[nHandle]->Texture = bmpList[nTexHandle];
	  ...
	  case kPSystem_SpiralArm:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed: SpiralArm", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 4000;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 12000;	// Whole system life
			theList[nHandle]->Scale = 0.1f*Scale;				// Small particles
			theList[nHandle]->bRunning = false;		// Create paused
			theList[nHandle]->bVisible = false;		// Create invisible
			theList[nHandle]->Texture = bmpList[nTexHandle];
		...
		case kPSystem_Trail:
		  nHandle = Allocate();
			...
			// Set particle system defaults
			theList[nHandle]->Lifetime = 6000;		// Seconds of life, per particle
			theList[nHandle]->SystemLife = 12000;	// Whole system life
		...
		case kPSystem_Guardian:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, GUARDIAN EFFECT", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 6000;		// Seconds of life, per particle
		...
		case kPSystem_ImplodeSphere:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, IMPLODESPHERE", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 2000;		// Seconds of life, per particle
	  ...
	  case kPSystem_ImplodeShockwave:
		  nHandle = Allocate();
			if(nHandle < 0)
			  {
				CCD->ReportError("PSAlloc failed, IMPLODE_SHOCKWAVE", false);
				return RGF_FAILURE;
				}
			// Set particle system defaults
			theList[nHandle]->Lifetime = 5000;		// Seconds of life, per particle
...
//	Pause
//
//	Pause and unpause a particle systems execution.  If there are
//	..particles in the system, they will STILL RENDER, but time will
//	..not advance for the system.
...
//	Hide
//
//	Hide and unhide a particle systems execution.  If there are
//	..particles in the system, they will STILL HAVE TIME ADVANCE,
//	..but they will NOT RENDER.
...
int CParticleSystem::Rewind(int psHandle)
{
  ...
  switch(theList[psHandle]->nType)
	  ...
	  case kPSystem_Shockwave:
			...
			theList[psHandle]->Lifetime = 5000;		// Seconds of life, per particle
			theList[psHandle]->SystemLife = 5000;	// Whole system life
	  ...
	  case kPSystem_Fountain:
			// Set particle system defaults
			theList[psHandle]->Lifetime = 5000;		// Seconds of life, per particle
			theList[psHandle]->SystemLife = 22000;	// Whole system life
			// Reload the particles
	  ...
	  case kPSystem_Rain:
			// Set particle system defaults
			theList[psHandle]->Lifetime = 5000;		// Seconds of life, per particle
			theList[psHandle]->SystemLife = 30000;	// Whole system life
			SetupRain(psHandle);
			break;
	  case kPSystem_Sphere:
			// Set particle system defaults
	  ...
	  case kPSystem_Column:
			// Set particle system defaults
			theList[psHandle]->Lifetime = 6000;		// Seconds of life, per particle
			theList[psHandle]->SystemLife = 16000;	// Whole system life
			SetupRain(psHandle);
			break;
		case kPSystem_ExplosiveArray:
			// Set particle system defaults
			theList[psHandle]->Lifetime = 4000;		// Seconds of life, per particle
			theList[psHandle]->SystemLife = 8000;	// Whole system life
			SetupRain(psHandle);
			break;
		case kPSystem_SpiralArm:
			// Reset particle system defaults
			theList[psHandle]->Lifetime = 4000;		// Seconds of life, per particle
			theList[psHandle]->SystemLife = 12000;	// Whole system life
			// Reload the particles
		...
		case kPSystem_Guardian:
			// Set particle system defaults
			theList[psHandle]->Lifetime = 6000;		// Seconds of life, per particle
			theList[psHandle]->SystemLife = 20000;	// Whole system life
...
//	..If bChangeExisting is true, _ALL_ particles in the system will
//	..change color.

int CParticleSystem::SetColor(int psHandle, GE_RGBA clrColor,
				bool bChangeExisting)
{
  if(theList[psHandle] == NULL)
...
//	..for particles that are used as projectile trails, spell effect
//	..trails, etc.

int CParticleSystem::SetEmitter(int psHandle, geVec3d emitterPos)
{
  if(theList[psHandle] == NULL)
	  return RGF_FAILURE;						// Try again, luser!
...
//	..bChangeExisting is true, _ALL_ particles in the system will
//	..change texture.

int CParticleSystem::SetTexture(int psHandle, int pmHandle,
					bool bChangeExisting)
{
  if(theList[psHandle] == NULL)
	  return RGF_FAILURE;						// Try again, luser!
...
int CParticleSystem::SetVelocity(int psHandle, float fVelocity,
			bool bChangeExisting)
{
  if(theList[psHandle] == NULL)
	  return RGF_FAILURE;						// Try again, luser!
...
//	..by the indicated particle system.  Existing particles are NOT
//	..effected.
...
theParticle *CParticleSystem::Spawn(int nHandle, GE_RGBA theColor, geVec3d Pos,
		geVec3d Rotation, float fVelocity)
{
  theParticle *theNewParticle = new theParticle;

	if(theNewParticle == NULL)
	  {
		CCD->ReportError("Particle Spawn: no memory", false);
...
theParticle *CParticleSystem::ReverseSpawn(int nHandle, GE_RGBA theColor, 
	geVec3d Pos, geVec3d Rotation, float fVelocity)
{
  theParticle *theNewParticle = new theParticle;
	geVec3d OldPosition, NewPosition, In;
	geXForm3d Xform;

	if(theNewParticle == NULL)
	  {
		CCD->ReportError("Reverse Particle Spawn: no memory", false);
    return NULL;								// Uh-oh, major problem.
		}
...
// Move particle based on the direction it's "facing"
  ...
  geVec3d_AddScaled(&OldPosition, &In, 
				fVelocity * theNewParticle->LifeTime/30, &NewPosition);
...
//	If out of time, and doesn't live forever, KILL THE MUTHA DEAD!
	...
	if((theList[nHandle]->SystemLife <= 0) &&
	   (theList[nHandle]->bForever == false))
	  {
		Clear(nHandle);												// System out of time
		theList[nHandle]->SystemLife = 0;			// Clamp to zero
	...
	if(theList[nHandle]->SpawnSpeed != 0)
	  {
		...
		float fTemp = (float)theList[nHandle]->SpawnSpeed / 1000.0f;
		fTemp = fTemp * (float)dwMsec;
...
void CParticleSystem::RenderSystem(int nHandle, geVec3d PlayerPos)
{
  theParticle *myParticle = theList[nHandle]->Particles;
...
// 100 particles for a shockwave
  ...
  theList[nHandle]->AlphaDecay = 255.0f / 
				(float)theList[nHandle]->Lifetime;		// Particle alpha decay speed
...
{
  static geVec3d vRot = {0, 0.0f, 0.0f};
	int nX, nY, nZ;
