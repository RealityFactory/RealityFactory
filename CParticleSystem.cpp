...
CParticleSystem::CParticleSystem()
{
		...
		pEntity=geEntity_EntitySetGetNextEntity(pSet, pEntity))
	{
		...
		{
			CCD->Log()->Warning("File %s - Line %d: %s: Failed to load particle map '%s'",
		...
		case kPSystem_ImplodeSpiralArm:
			{
				...
				{
					CCD->Log()->Warning("Failed to create particle system from proxy");
		...
		default:
			CCD->Log()->Warning("Illegal particle system style");
...
int CParticleSystem::LoadParticleMap(int *pmHandle, const char *szPrimary, const char *szAlpha)
{
	...
	{
		CCD->Log()->Warning("File %s - Line %d: Failed to open particlemap file '%s'",
	...
	case kPSystem_Shockwave:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, SHOCKWAVE");
				return RGF_FAILURE;
			}
	...
	case kPSystem_Fountain:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, FOUNTAIN");
				return RGF_FAILURE;
			}
			// Set particle system defaults
			theList[nHandle]->Lifetime			= 5000.0f;			// Seconds of life, per particle
	...
	case kPSystem_Rain:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, RAIN");
				return RGF_FAILURE;
			}
			// Set particle system defaults
			theList[nHandle]->Lifetime			= 5000.0f;			// Seconds of life, per particle
	...
	case kPSystem_Sphere:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, SPHERE");
				return RGF_FAILURE;
			}
			// Set particle system defaults
	...
	case kPSystem_Column:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, COLUMN");
				return RGF_FAILURE;
			}
			// Set particle system defaults
	...
	case kPSystem_ExplosiveArray:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, EXPLOSIVE ARRAY");
				return RGF_FAILURE;
			}
			// Set particle system defaults
	...
	case kPSystem_SpiralArm:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed: SpiralArm");
				return RGF_FAILURE;
			}
			// Set particle system defaults
	...
	case kPSystem_Guardian:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, GUARDIAN EFFECT");
				return RGF_FAILURE;
			}
			// Set particle system defaults
			theList[nHandle]->Lifetime			= 6000.0f;			// Seconds of life, per particle
	...
	case kPSystem_ImplodeSphere:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, IMPLODESPHERE");
				return RGF_FAILURE;
			}
			// Set particle system defaults
			theList[nHandle]->Lifetime			= 2000.0f;			// Seconds of life, per particle
	...
	case kPSystem_ImplodeShockwave:
		{
			...
			if(nHandle < 0)
			{
				CCD->Log()->Warning("PSAlloc failed, IMPLODE_SHOCKWAVE");
				return RGF_FAILURE;
			}
			// Set particle system defaults
			theList[nHandle]->Lifetime			= 5000.0f;			// Seconds of life, per particle
									...
									float fVelocity)
{
	...
	{
		CCD->Log()->Warning("Particle Spawn: out of memory");
										   ...
										   float fVelocity)
{
	...
	if(theNewParticle == NULL)
	{
		CCD->Log()->Warning("Reverse Particle Spawn: out of memory");
		return NULL;								// Uh-oh, major problem.
	}
