...
int Collider::GetContentsOf(geVec3d Position, geExtBox *theBox, 
...
// GetContentsOf gets the contents of an ExtBox zone, with an optional
//	..actor to exclude
...
bool Collider::CheckActorCollision(geVec3d Start, geVec3d End, geActor *theActor)
{
	//	First, check to see if the bounding box will hit, and if not, we
	//	..need not go to a Stage II collision test
...
bool Collider::CheckForCollision(geVec3d *Min, geVec3d *Max,
								 geVec3d OldPosition, geVec3d NewPosition)
{
	GE_Collision Collision;
...
bool Collider::CheckForCollision(geVec3d *Min, geVec3d *Max,
								 geVec3d OldPosition, geVec3d NewPosition, GE_Contents *Contents)
{
	GE_Collision Collision;
	int Result, Result2;
...
bool Collider::CanOccupyPosition(geVec3d *thePoint, geExtBox *theBox)
{
	GE_Contents Contents;
...
geBoolean Collider::CheckIntersection(geVec3d *Position, geActor *Actor, geWorld *World)
{
	int TotalBoneCount = 0;
	geExtBox theBoneBox;
	geBoolean Collided = GE_FALSE;
...
geBoolean Collider::CheckSubCollision(geVec3d *Start, geVec3d *End, geActor *Actor, 
									  geWorld *World)
{
	int TotalBoneCount = 0;
	geExtBox theBoneBox;
									  ...
									  geWorld *World, GE_Collision *Collision)
{
	...
	for(int BoneIndex = 0; BoneIndex < TotalBoneCount; BoneIndex++)
	{
		...
		// ..the CURRENT POSITION of the root bone of the actor.
		xStart = lStart; xEnd = lEnd;
		geVec3d gTemp; memcpy(&gTemp, Start, sizeof(geVec3d));
		gTemp.X -= theTransform.Translation.X;
		gTemp.Y -= theTransform.Translation.Y;
		gTemp.Z -= theTransform.Translation.Z;
		xStart.X -= gTemp.X; xStart.Y -= gTemp.Y; xStart.Z -= gTemp.Z;
...
//=====================================================================================
void Collider::GetMoveBox(const geVec3d *Mins, const geVec3d *Maxs, const geVec3d *Front, const geVec3d *Back, geVec3d *OMins, geVec3d *OMaxs)
{
	int32		i;
...
//	..return collision information.  Useful for pre-checking
//	..AI motion.

geBoolean Collider::Probe(geXForm3d theXForm, float fDistance, GE_Collision *theCollision)
{
	geVec3d Start, End;
	geVec3d InVector;
...
//	Given a transform, probe forward/backward from it some distance and
//	..return collision information.  Useful for pre-checking
//	..AI motion.  This flavor takes an Actor pointer to exclude from the checking.
...
// Weapon
int Collider::ProcessCollision(GE_Collision theCollision, geActor *theActor, bool Gravity)
{
	bool bShoot = false;
...
bool Collider::CheckActorCollisionD(geVec3d Start, geVec3d End, geActor *theActor,
								   GE_Collision *Collision)
{
	//	First, check to see if the bounding box will hit, and if not, we
	//	..need not go to a Stage II collision test
...
bool Collider::CanOccupyPositionD(geVec3d *thePoint, geExtBox *theBox,
								 geActor *Actor, GE_Contents *Contents)
{
	memset(Contents, 0, sizeof(GE_Contents));
...
bool Collider::CheckForBoneCollision(geVec3d *Min, geVec3d *Max,
								 geVec3d OldPosition, geVec3d NewPosition, GE_Collision *Collision,
								 geActor *Actor, char *BoneHit, bool BoneLevel, void *CollisionObject )
{
	GE_Contents Contents;
	int Result, Result1;
