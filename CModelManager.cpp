...
int CModelManager::SetParentModel(geWorld_Model *theModel, geWorld_Model *ParentModel)
{
   ...
   //Find the Parent Model and set its child pointer, or if a child has already been set then add to the child's sibling pointer list
...
int CModelManager::SetTargetTime(geWorld_Model *theModel, geFloat TT)
{
   ...
   else
   {
      if (pEntry->bHasMoved)
      {  //Restart
         geFloat TickTime = (float) (TT * 1000.0);
         pEntry->TargetTime = TickTime;
         if (TickTime < pEntry->ModelTime)
...
int CModelManager::GetTargetTime(geWorld_Model *theModel, geFloat *TT)
{
   ...
   *TT = (float) (pEntry->TargetTime / 1000.0);
...
int CModelManager::GetModelCurrentTime(geWorld_Model *theModel, geFloat *time)
{
   ...
   *time = (float) (pEntry->ModelTime / 1000.0);
...
{
   //Start Aug2003DCS
   memset(&ModelState[0], NOT_PROCESSED, 512 * sizeof(int));
   //End Aug2003DCS
   ...
   if (MainList[nEntry] != NULL)
   {
      ...
      if (MainList[nEntry]->bMoving)
      {
         ...
         if(MainList[nEntry]->bForward)
         {
            ...
            //Start Aug2003DCS
            if (MainList[nEntry]->TargetTime >= 0.0)
            {
...
int CModelManager::AddNewModelToList(geWorld_Model *theModel, int nModelType)
{
   ...
   for(int nTemp = 0; nTemp < 512; nTemp++)
   {
      ...
      if(MainList[nTemp] == NULL)
      {
         ...
         //Start Aug2003DCS
         if (nTemp >= ManagedModels) ManagedModels = nTemp + 1;
         //End Aug2003DCS
         return RGF_SUCCESS;
      }
   }
