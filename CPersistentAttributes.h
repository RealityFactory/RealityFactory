...
public:
  ...
  int Set(char *szTag, int nValue);	// Set attribute value
	int SetIf(char *szTag, int nHow, int nCompareValue, int nValue);
																	// Set attribute value if test condition true
	int Modify(char *szTag, int nValue);
																	// Modify value of attribute
	int ModifyIf(char *szTag, int nHow, int nCompareValue, int nValue);
																	// Modify if test condition true
	int Add(char *szTag);						// Add an instance of an attribute
  int AddAndSet(char *szTag, int nValue);	// Add attribute with value
	int AddIf(char *szTag, int nHow, int nCompareValue, char *szWhat);
																	// Add instance if test condition true
	int Remove(char *szTag);				// Remove an instance of an attribute
	int RemoveIf(char *szTag, int nHow, int nCompareValue);
																	// Remove instance if test condition true
...
// end change RF064
  ...
  int AllocateUserData(char *szTag, int nDataSize);
