...
// Version 053
  ...
  int DisplayFrameAt(int XPos, int YPos, DWORD dwTime);
															// Display one frame from a specific time
	int DisplayFrame(int XPos, int YPos, int FrameID);
															// Display one specific frame
  ...
  int DisplayFrameTexture(int nFrame, char *szTextureName);	
															// Display AVI frame as texture
  int DisplayNextFrameTexture(char *szTextureName,	bool bFirstFrame);
															// Play an AVI as a texture map
private:
