...
{
	geEntity_EntitySet *pSet;//, *pSetSun;
	geEntity *pEntity;//, *pEntitySun;
	...
	pEntity= geEntity_EntitySetGetNextEntity(pSet, pEntity)) 
	{
		...
		if(!IsInList(pMesh->szActorFile, &Index))
		{
			if(CCD->FileExist(kActorFile, pMesh->szActorFile))
			{
				...
				{
					char szError[256];
					sprintf(szError,"CStaticMesh: can't create VertMatList from file '%s'", pMesh->szActorFile);
			...
			{
				char szError[256];
				sprintf(szError,"CStaticMesh: file '%s' doesn't exist", pMesh->szActorFile);
				CCD->ReportError(szError, false);
				CCD->ShutdownLevel();
				delete CCD;
				CCD = NULL;
...
/*		if(pMesh->CompSunLightMin>100)
		...
		geVec3d_Scale(&pMesh->ActorRotation, 0.0174532925199433f, &pMesh->ActorRotation);
	...
	for(int LOD=0;LOD<4;LOD++)
	{
		...
		if(ActorFile)
		{
			...
			if(!ActorDef)
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't create def from file '%s'", Name);
				CCD->ReportError(szError, false);
				if(!LOD)
				{
		...
		else							// File didn't open, error out.
		{
			...
			{
				char szError[256];
				sprintf(szError,"CStaticMesh: can't load actor '%s'", Name);
				CCD->ReportError(szError, false);
				if(MeshList[m_MeshCount]->szFilename != NULL)
					delete MeshList[m_MeshCount]->szFilename;
				delete MeshList[m_MeshCount];
		...
		if(!B)
		{
			...
			char szError[256];
			sprintf(szError,"CStaticMesh: can't get body from actor '%s'", Name);
			CCD->ReportError(szError, false);
		...
		{ 
			...
			if(!(MeshList[m_MeshCount]->Bitmaps[LOD]))
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't allocate bitmaps");
				CCD->ReportError(szError, false);
			...
			for(i=0;i<(MeshList[m_MeshCount]->MaterialCount[LOD]);i++)
			{
				...
				{
					char szError[256];
					sprintf(szError,"CStaticMesh: mname %s", MaterialName);
					CCD->ReportError(szError, false);
				}
			}
			...
			if(!MeshList[m_MeshCount]->Verts[LOD])
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't allocate verts");
				CCD->ReportError(szError, false);
			...
			if(!MeshList[m_MeshCount]->FaceI[LOD])
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't allocate faceindex");
				CCD->ReportError(szError, false);
			...
			if(!MeshList[m_MeshCount]->VNormalI[LOD])
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't allocate normalindex");
				CCD->ReportError(szError, false);
			...
			if(!MeshList[m_MeshCount]->VNormals[LOD])
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't allocate normals");
				CCD->ReportError(szError, false);
			...
			if(!MeshList[m_MeshCount]->Color[LOD])
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't allocate color");
				CCD->ReportError(szError, false);
			...
			if(!MeshList[m_MeshCount]->MaterialI[LOD])
			{
				...
				char szError[256];
				sprintf(szError,"CStaticMesh: can't allocate materialindex");
				CCD->ReportError(szError, false);
...
void CStaticMesh::ComputeLighting(StaticMesh *pMesh, void* pLight, int LType)
{
	...
	if(LType == LIGHT_SUNLIGHT || LType == LIGHT_SPOTLIGHT)
	{
		...
		geXForm3d_SetZRotation(&temp, Angles.Z*0.0174532925199433f);
		geXForm3d_RotateX(&temp, Angles.X*0.0174532925199433f);
		geXForm3d_RotateY(&temp, Angles.Y*0.0174532925199433f);	
...
// changed QD 09/28/2004
geBoolean CStaticMesh::RayTracing(StaticMesh *CallingMesh, int LOD, geVec3d OldPosition, geVec3d NewPosition, GE_Collision *Collision)
{
...
bool CStaticMesh::CollisionCheck(geVec3d *Min, geVec3d *Max, geVec3d OldPosition, geVec3d NewPosition, GE_Collision *Collision)
{
