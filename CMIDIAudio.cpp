...
CMIDIAudio::CMIDIAudio()
{
  ...
  m_ListCount = 0;				// No list of files to play
	...
	m_savefile[0] = m_MIDIFile[0] = 0;			// No file yet
...
CMIDIAudio::~CMIDIAudio()
{
  ...
  if(theError != 0)
		CCD->ReportError("CMIDIAudio: Failed to close MCI MIDI SEQUENCER device",
					false);
...
//	..is the default sequencer as set in the Multimedia control panel.
  ...
  if(mciSendCommand(NULL, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_ELEMENT,
        (DWORD)(LPVOID) &mciOpen))
    {
		...
		char szBug[256];
		sprintf(szBug, "CMIDIAudio: can't open sequence file '%s'\n", szFile);
  ...
  if(mciSendCommand(m_mciDeviceID, MCI_PLAY, MCI_NOTIFY, (DWORD)(LPVOID)&mciPlay))
	  {
		// Can't play it?  Damn!
		char szBug[256];
		sprintf(szBug, "CMIDIAudio: can't play sequence file '%s'\n", szFile);
		CCD->ReportError(szBug, false);
	  mciClose.dwCallback = NULL;							// Please, no notifications
	...
	if(m_szList == NULL)
	  {
		...
		CCD->ReportError("CMIDIAudio: playlist alloc fail #1", false);
		return RGF_FAILURE;
		}
  ...
  for(int nTemp = 0; nTemp < nCount; nTemp++)
	  {
		...
		if(m_szList[nTemp] == NULL)
		  {
			// Worse and worse!
			CCD->ReportError("CMIDIAudio: playlist alloc fail #2", false);
			return RGF_FAILURE;
			}
...
int CMIDIAudio::Stop()
{
  ...
  if(theError != 0)
		CCD->ReportError("CMIDIAudio: Failed to stop MCI MIDI SEQUENCER device",
				false);
  ...
  if(m_bLooping)
    ...
    if(mciSendCommand(m_mciDeviceID, MCI_SEEK, MCI_WAIT | MCI_SEEK_TO_START,
      (DWORD)(LPMCI_SEEK_PARMS)&mciSeek))
			{
			CCD->ReportError("CMIDIAudio: Can't loop to start of MIDI File", false);
			return;
			}
    memset(&mciPlay, 0, sizeof(MCI_PLAY_PARMS));
    if(mciSendCommand(m_mciDeviceID, MCI_PLAY, MCI_NOTIFY, (DWORD)(LPVOID)&mciPlay))
	    {
		  ...
		  CCD->ReportError("CMIDIAudio: can't restart looping MIDI file", false);
	    mciClose.dwCallback = NULL;							// Please, no notifications
      mciSendCommand(m_mciDeviceID, MCI_CLOSE, MCI_WAIT, (DWORD)&mciClose);
			m_bLooping = false;
		  return;
