...
//	Constructor
//
//	Initialize all path followers and set the path origin and next
//	..target points.
...
//	Ok, dig through 'em all.
	...
	for(pEntity= geEntity_EntitySetGetNextEntity(pSet, NULL); pEntity;
		...
		pFollower->TimeInMotion = 0;					// Not in motion yet
		...
		if(szType == NULL)
		  {
			...
			char szBug[132];
			sprintf(szBug, "PathFollower can't find bound entity '%s' in registry",
						pFollower->EntityName);
		...
		CCD->PathDatabase()->Locate(pFollower->PathFirstNode, 
			&pFollower->CurrentNodeType, 	&pFollower->PathOrigin, 
		...
		if(pFollower->CurrentNodeType != RGF_POINT_TYPE_RANGED)
		  {
			...
			CCD->PathDatabase()->NextPoint(pFollower->PathHandle,
								&pFollower->CurrentTarget);
			}
		else
		  {
...
// Effect
			case 0:							// MorphingField
			  if(CCD->MorphingFields()->BindToPath(pFollower->EntityName) != RGF_SUCCESS)
				  {
					char szBug[128];
					sprintf(szBug,"Can't bind MorphingField entity '%s' to path '%s'",
			...
			case 1:							// StaticEntityProxy
			  if(CCD->Props()->BindToPath(pFollower->EntityName) != RGF_SUCCESS)
				  {
					char szBug[128];
					sprintf(szBug,"Can't bind StaticEntityProxy entity '%s' to path '%s'",
						pFollower->EntityName, pFollower->PathFirstNode);
					CCD->ReportError(szBug, false);
					}
			  break;
			case 2:							// TeleportTarget
			  if(CCD->Teleporters()->BindToPath(pFollower->EntityName) != RGF_SUCCESS)
				  {
					char szBug[128];
					sprintf(szBug,"Can't bind TeleportTarget entity '%s' to path '%s'",
						pFollower->EntityName, pFollower->PathFirstNode);
					CCD->ReportError(szBug, false);
					}
			  break;
			default:
				{
				char szBug[128];
				sprintf(szBug,"Attempt to bind nonbindable entity '%s' type '%s' to path",
...
//	Ok, dig through 'em all.
	...
	for(pEntity= geEntity_EntitySetGetNextEntity(pSet, NULL); pEntity;
	  ...
	  if(pFollower->RangeTrigger == 0.0)
		...
		pFollower->TimeInMotion = 0;									// Clear time in motion
...
//	Ok, dig through 'em all.
	...
	for(pEntity= geEntity_EntitySetGetNextEntity(pSet, NULL); pEntity;
		...
		*NextPosition = pFollower->CurrentPosition;
		...
		if(nMotionTime > 100.0)
		  nMotionTime = 100.0;			// In case of stalls elsewhere
		geFloat fDistance = nMotionTime * (pFollower->Speed / 1000.0f);
		...
		switch(pFollower->CurrentNodeType)
		  {
			...
			case RGF_POINT_TYPE_WAY:
				...
				geVec3d_Subtract(&pFollower->CurrentTarget, &pFollower->CurrentPosition,
						&DirectionVector);
				...
				if(pFollower->TimeInMotion > pFollower->RandomRollTime)
				  {
					...
					pFollower->CurrentTarget = RandomPosition;
					pFollower->TimeInMotion = 0;				// Clear re-seek time
					}
		...
		*NextPosition = temp;						// Copy out for use
		...
		if(geVec3d_DistanceBetween(&pFollower->CurrentPosition, 
				&pFollower->CurrentTarget) <= pFollower->Speed)
			{
...
int CPathFollower::GetDirectionVector(geVec3d LookFrom, geVec3d LookAt,
...
int CPathFollower::GetRotationToFacePoint(geVec3d LookFrom, geVec3d LookAt,
				geVec3d *LookRotation)
{
	...
	if(l > 0.0f) 
	  {
		x = LookRotation->X;
		LookRotation->X = (geFloat)( GE_PI*0.5 ) - 
					(geFloat)acos(LookRotation->Y / l);
