...
CCDAudio::CCDAudio()
{
	...
	if(theError != 0)
	{
		CCD->ReportError("CCDAudio: Failed to open MCI CD AUDIO device", false);
		return;
	}
	else
	{
...
CCDAudio::~CCDAudio()
{
	...
	if(theError != 0)
		CCD->ReportError("CCDAudio: Failed to close MCI CD AUDIO device", false);
...
int CCDAudio::Play(int nTrack, bool bLoop)
{
	...
	{
		char szError[80];
		sprintf(szError, "CCDAudio: can't play from %d to %d\n", nTrack, nTrack+1);
...
int CCDAudio::Stop()
{
	...
	if(theError != 0)
	{
		CCD->ReportError("CCDAudio: Failed to stop MCI CD AUDIO device", false);
		return RGF_FAILURE;
	}
