...
public:
  ...
  int LoadParticleMap(int *pmHandle, char *szPrimary,
							char *szAlpha);		// Load bitmap for particles
	int Create(int *psHandle, int kSystemType, int nTexHandle, geVec3d emitterPos, float Scale,
				char *szName);					// Create particle system
	int CreateSound(geVec3d Origin, char *SoundFile, float radius);
  ...
  int Hide(int psHandle, bool bShowIt);				// Hide/unhide particle system
	...
	int SetColor(int psHandle, GE_RGBA clrColor, bool bChangeExisting);	
																	// Set particle color
	int SetEmitter(int psHandle, geVec3d emitterPos);	// Set emitter position
	int SetTexture(int psHandle, int pmHandle, bool bChangeExisting);
																	// Set particle texture
	int SetVelocity(int psHandle, float fVelocity, bool bChangeExisting);
...
private:
  ...
  int Clear(int nHandle);					// Clear all particles from system
	theParticle *Spawn(int nHandle, GE_RGBA theColor,
				geVec3d Pos, geVec3d Rotation, float fVelocity);
																	// Add new particle to system
  theParticle *ReverseSpawn(int nHandle, GE_RGBA theColor, 
				geVec3d Pos, geVec3d Rotation, float fVelocity);
																	// Add new imploding particle to system
	void Sweep(int nHandle, float dwMsec);	// Update particle system
  void ApplyGravity(int nHandle, theParticle *theParticle);
  void RenderSystem(int nHandle, geVec3d PlayerPos);				// Render particle system
