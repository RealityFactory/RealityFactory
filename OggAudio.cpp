...
int OggAudio::Load(char *szFileName)
{
	...
	if (vf) {
		if (ov_open(vf,&ovf,NULL,0)<0)  {
			CCD->ReportError("[OggAudio::Load]:ov_open failed", false);
			return RGF_FAILURE;
		}
	}
	else {
		CCD->ReportError("[OggAudio::Load]:unable to find file", false);
		return RGF_FAILURE;
	}
	...
	{
		CCD->ReportError("[OggAudio::Load]:Error while creating soundbuffer", false);
		ov_clear(&ovf);
		fclose(vf);
		vf=NULL;
		return RGF_FAILURE;
	...
	if(nTimer == 0)
	{
		CCD->ReportError("[OggAudio::Load]:Timer startup failure", false);
...
int OggAudio::Play(bool bLooping)
{
	//	Check for stream availability
	if(m_pStream == NULL) {
		CCD->ReportError("[OggAudio::Play]:no stream", false);
...
int OggAudio::Stop()
{
	//	Check for stream availability
	if(m_pStream == NULL){
		CCD->ReportError("[OggAudio::Stop]:no stream", false);
		return RGF_FAILURE;									// No stream
	}
...
int OggAudio::Pause()
{
	//	Check for stream availability
	if(m_pStream == NULL) {
		CCD->ReportError("[OggAudio::Pause]:no stream", false);
		return RGF_FAILURE;									// No stream
	}
...
int OggAudio::Delete()
{
	//	Check for stream availability
	if(m_pStream == NULL) {
		CCD->ReportError("[OggAudio::Delete]:no stream", false);
		return RGF_FAILURE;							// No stream
	}
...
int OggAudio::Rewind()
{
	//	Check for stream availability
	if(m_pStream == NULL){
		CCD->ReportError("[OggAudio::Rewind]:no stream", false);
		return RGF_FAILURE;									// No stream
	}
	...
	if(m_fActive == FALSE) {
		CCD->ReportError("[OggAudio::Rewind]:attempting to rewind an inactive stream", false);
...
int OggAudio::SetVolume(int nVolume)
{
	...
	if     (m_pStream == NULL) {
		CCD->ReportError("[OggAudio::SetVolume]:no stream", false);
