...
//	Set
//
//	Locate the attribute identified by the tag and set its value to
//	..the desired value.  If the attribute doesn't exist, create it
//	..and set the value.
...
//	..and if the result is TRUE, set the attributes value to the desired
//	..value.
...
//	..'nHow' and if true modify the value by adding 'nValue' to the attribute
//	..value.
...
//	Dump
//
//	Dump the contents of the attribute list to the debug window and
//	..the debug file.  Typically used for debugging.
...
void CPersistentAttributes::Dump()
{
	...
	while(pTemp != NULL)
	{
		sprintf(szDump, "%s count %d value %d", pTemp->Name,
			pTemp->Count, pTemp->Value);
